    ...
    const initData = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          // Set default roster selection if needed
          if (rosterData.length > 0) {
             setNewEvent(prev => {
                 // Only set if currently empty to avoid overwriting user input during re-renders
                 return prev.rosterId ? prev : { ...prev, rosterId: rosterData[0].id };
             });
          }
        }
      } catch (error) {
                ...
                <ScrollView>
                    ...
                    <View style={styles.row}>
                        {['Practice', 'Morale'].map(t => (
                            <TouchableOpacity 
                                key={t}
                                style={[styles.typeBtn, newEvent.type === t && styles.activeType]}
                    ...
                    <Input 
                        ...
                        onChangeText={t => setNewEvent({...newEvent, dateTime: t})}
                        placeholder="2025-10-25T18:00"
                    />
